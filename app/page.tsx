...
export default function Page() {
  ...
  async function submitEmployee(event: FormEvent<HTMLFormElement>) {
    ...
    const { error: insertError } = await getSupabase().from('EMPLOYEE').insert({ NAMA_KARYAWAN: form.get('name'), KODE_KARYAWAN: form.get('code'), TANGGAL_LAHIR: form.get('birth'), ALAMAT: form.get('address') })
      ...
      <section className="content">
        ...
        <div className="workspace">
          <div className="panel form-panel"><div className="panel-heading"><div><p className="eyebrow">INPUT TRANSAKSI</p><h2>{active === 'employee' ? 'Master Karyawan' : active === 'salary' ? 'Transaksi Salary' : 'Transaksi Bonus'}</h2></div><span className="status-pill"><span />Terhubung</span></div>
            ...
            {active === 'bonus' && <form onSubmit={submitBonus}><div className="formula-card"><Calculator size={22} /><div><strong>Formula bonus standar</strong><span>BONUS = SALARY periode × 5%</span></div><b>5%</b></div><label>Transaksi salary<select name="salaryId" required defaultValue=""><option value="" disabled>Pilih salary periode</option>{salaries.data?.map((salary) => <option key={salary.ID_SALARY} value={salary.ID_SALARY}>#{salary.ID_SALARY} · {months[salary.BULAN - 1]} {salary.TAHUN} · {money.format(salary.SALARY)}</option>)}</select></label><Submit busy={busy} label="Hitung & simpan bonus" /></form>}
